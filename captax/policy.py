...
def create_policies(env):
    ...
    for filename in policy_parameters_files:
        ...
        produce_policy_perspective_run = _produce_policy_perspective_run(policy_name, perspective)
...
class Policy():
    ...
    policy_parameters_permitted_ranges_file : str
        String describing the file path of file storing permitted ranges for policy parameters.
    tax_rates : dict
        Tax rate parameters.
    deduction : dict
    ...
    def __init__(self, policy_name, perspective, env):
        ...
        This method calls five other methods:
            * self._read_policy_parameters_file() : Reads the policy parameters file, which includes
                policy parameters that vary by year.
            * self._read_depreciation_parameters() : Reads in various policy parameters related to
                calculations of tax depreciation.
            * self._read_investment_tax_credit_parameters() : Reads in parameters related to the
                investment tax credit.
            * self._read_tax_rate_adjustment_parameters() : Reads in parameters related to tax rate
                adjustments.
            * self._check_policy_parameter_ranges() : Checks that all policy parameters are within
                permitted ranges.
        ...
        policy_name : str
            String describing the policy being considered.
        perspective : str
            String describing the perspective being used (must be "comprehensive" or "uniformity").
        env : Environment object
            Economic environment parameters.
        ...
        -------
        None
            Method reads policy parameters and stores them as attributes of Policy object.
        ...
        self.policy_parameters_permitted_ranges_file = f'{self.policy_path}/policy_parameters_permitted_ranges.csv'
        ...
        self.holding_period_changes = {
            'cap_gains' : {
                'short_term': pol['change_cap_gains_short_term_holding_period'].to_numpy(),
                'long_term': pol['change_cap_gains_long_term_holding_period'].to_numpy(),
            ...
            'ret_plan' : {
                ...
                'nontaxable': pol['change_ret_plan_nontaxable_holding_period'].to_numpy()
            },
        ...
        # parameter values specified in environment parameter file fall within permitted ranges.
        base_yr_keys = ['cap_gains_short_term_share',
        ...
        for key in base_yr_keys:
            ...
            if not (
                (self.env.permitted_ranges[key]['min_value'] <= (self.env.env[key].values + pol[change_key])).all()
                & ((self.env.env[key].values + pol[change_key]) <= self.env.permitted_ranges[key]['max_value']).all()
            ):
                raise ValueError(f"One of the parameter values of {change_key} is such that the sum of ({key} + "
                                 f"{change_key}) is not within the permitted range {permitted_range_str}'")

        # Policy suffixes pointing to depreciation, investment tax credit & tax rate adjustment parameters
        file_policy_suffixes = {
            ...
            'tax_rate_adjustments' : {
                ...
                'c_corp' : {
                    ...
                    'industry_adjustments': pol['c_corp_industry_adjustments'].to_numpy()
                },
                'pass_thru' : {
                    'asset_adjustments': pol['pass_thru_asset_adjustments'].to_numpy(),
                    'industry_adjustments': pol['pass_thru_industry_adjustments'].to_numpy()
                },
            },
        }
        ...
        self.account_category_shares = {
            'c_corp': {
                'equity' : {
                    'taxable' : pol['c_corp_equity_account_category_share_taxable'].to_numpy(),
                    'deferred' : pol['c_corp_equity_account_category_share_deferred'].to_numpy(),
                    'nontaxable' : pol['c_corp_equity_account_category_share_nontaxable'].to_numpy()
                },
                'debt' : {
                    'taxable' : pol['c_corp_debt_account_category_share_taxable'].to_numpy(),
                    'deferred' : pol['c_corp_debt_account_category_share_deferred'].to_numpy(),
                    'nontaxable' : pol['c_corp_debt_account_category_share_nontaxable'].to_numpy()
                },
            },
            'pass_thru' : {
                'equity' : {
                    'taxable' : pol['pass_thru_equity_account_category_share_taxable'].to_numpy(),
                    'deferred' : pol['pass_thru_equity_account_category_share_deferred'].to_numpy(),
                    'nontaxable' : pol['pass_thru_equity_account_category_share_nontaxable'].to_numpy()
                },
                'debt' : {
                    'taxable' : pol['pass_thru_debt_account_category_share_taxable'].to_numpy(),
                    'deferred' : pol['pass_thru_debt_account_category_share_deferred'].to_numpy(),
                    'nontaxable' : pol['pass_thru_debt_account_category_share_nontaxable'].to_numpy()
                },
            },
            'ooh' : {
                'equity' : {
                    'taxable' : pol['ooh_equity_account_category_share_taxable'].to_numpy(),
                    'deferred' : pol['ooh_equity_account_category_share_deferred'].to_numpy(),
                    'nontaxable' : pol['ooh_equity_account_category_share_nontaxable'].to_numpy()
                },
                'debt' : {
                    'taxable': pol['ooh_debt_account_category_share_taxable'].to_numpy(),
                    'deferred': pol['ooh_debt_account_category_share_deferred'].to_numpy(),
                    'nontaxable': pol['ooh_debt_account_category_share_nontaxable'].to_numpy()
                },
            },
        }
        ...
        self.depreciation = {
            'recovery_periods' : self._read_depreciation_parameters(
                'recovery_periods_',
                file_policy_suffixes['recovery_periods']
            ),
            'acceleration_rates' : self._read_depreciation_parameters(
                'acceleration_rates_',
                file_policy_suffixes['acceleration_rates']
            ),
            'straight_line_flags' : self._read_depreciation_parameters(
                'straight_line_flags_',
                file_policy_suffixes['straight_line_flags']
            ),
            'inflation_adjustments' : self._read_depreciation_parameters(
                'inflation_adjustments_',
                file_policy_suffixes['inflation_adjustments']
            ),
            ...
            'other_expens_shares' : self._read_depreciation_parameters(
                'other_expens_shares_',
                file_policy_suffixes['other_expens_shares']
            ),
        }
        ...
        self.itc = {
            'rates' : self._read_investment_tax_credit_parameters(
                'itc_rates_',
                file_policy_suffixes['itc']['rates']
            ),
            'nondeprcbl_bases' : self._read_investment_tax_credit_parameters(
                'itc_nondeprcbl_bases_',
                file_policy_suffixes['itc']['nondeprcbl_bases']
            ),
        }
        ...
        self.tax_rate_adjustments = {
            'c_corp': {
                'asset_type' : self._read_tax_rate_adjustment_parameters(
                    'asset_adjustments_',
                    file_policy_suffixes['tax_rate_adjustments']['c_corp']['asset_adjustments']
                ),
                'industry' : self._read_tax_rate_adjustment_parameters(
                    'industry_adjustments_',
                    file_policy_suffixes['tax_rate_adjustments']['c_corp']['industry_adjustments']
                ),
            },
            'pass_thru' : {
                'asset_type' : self._read_tax_rate_adjustment_parameters(
                    'asset_adjustments_',
                    file_policy_suffixes['tax_rate_adjustments']['pass_thru']['asset_adjustments']
                ),
                'sec_199A' : self._read_tax_rate_adjustment_parameters(
                    'sec_199A_adjustments_',
                    file_policy_suffixes['tax_rate_adjustments']['sec_199A_adjustments']
                ),
                'industry' : self._read_tax_rate_adjustment_parameters(
                    'industry_adjustments_',
                    file_policy_suffixes['tax_rate_adjustments']['pass_thru']['industry_adjustments']
                ),
            },
        }
        ...
        #---------------------------------------------------------------------------------
        permitted_ranges = pd.read_csv(self.policy_parameters_permitted_ranges_file,
                                       index_col='policy_parameter').to_dict('index')
        self._check_policy_parameter_ranges(pol, permitted_ranges)

        print(f'* Policy parameters for {policy_name} with the {perspective} perspective read')

        return None


    def _read_policy_parameters_file(self, filename):
        """Read the policy parameters file, which includes policy parameters that vary by year.

        Examples of policy parameters are tax rates on C corps income and shares of total interest
        deductible by pass-throughs.
        ...
        policy_params : pd.DataFrame
            DataFrame with all policy parameter values selected for the number of years specified
            by the NUM_YEARS constant.

        """
        df = pd.read_csv(filename, index_col=0)

        if NUM_YEARS > len(df):
            raise ValueError(f'NUM_YEARS is greater than the number of rows in {filename}')
        else:
            # Only select policy parameters for the number of years specified in NUM_YEARS
    ...
    def _read_depreciation_parameters(self, filename, filename_suffix):
        """Read in various policy parameters related to calculations of tax depreciation.
        ...
        depreciation_parameters : np.ndarray
            Array of policy depreciation parameters by detailed industry, asset type, and year.

        """
        # Initialize array
        depreciation_parameters = np.zeros((NUM_DETAILED_INDS,
                                            NUM_ASSETS,
                                            NUM_YEARS))
        ...
        for i_year in range(NUM_YEARS):
            ...
            data = pd.read_csv(self.policy_path
            ...
            depreciation_parameters[:NUM_DETAILED_INDS, :NUM_ASSETS, i_year] = data.to_numpy()
    ...
    def _read_investment_tax_credit_parameters(self, filename, filename_suffix):
        ...
        """
        # Initialize array
        itc_parameters = np.zeros((NUM_INDS,
                                   NUM_ASSETS,
                                   NUM_YEARS))
        ...
        for i_year in range(NUM_YEARS):
                ...
                data = pd.read_csv(self.policy_path
                                   ...
                                   + filename + filename_suffix[i_year]
                                   + '.csv')
                data.drop([0], axis=0, inplace=True)
    ...
    def _read_tax_rate_adjustment_parameters(self, filename, filename_suffix):
        ...
        There are 3 files that are read in, one of which is a matrix of asset type values:
            * asset_adjustments
        The other two are matrices of detailed industry values:
            * sec_199A_adjustments
            * industry_adjustments

        All matrices include two columns, one for the eligibility parameters and one for the rate
        parameter.
        ...
        if filename in ['sec_199A_adjustments_', 'industry_adjustments_']:
            tax_rate_adjustment_parameters = np.zeros((NUM_DETAILED_INDS,
                                                       NUM_TAX_RATE_ADJUSTMENTS_COMPONENTS,
                                                       NUM_YEARS))
        elif filename == 'asset_adjustments_':
            tax_rate_adjustment_parameters = np.zeros((NUM_ASSETS,
                                                       NUM_TAX_RATE_ADJUSTMENTS_COMPONENTS,
                                                       NUM_YEARS))
        else:
            raise ValueError('tax rate adjustment file name must be equal to \
        ...
        for i_year in range(NUM_YEARS):
            ...
            tax_rate_adjustment_parameters[:NUM_DETAILED_INDS,
                                           :NUM_TAX_RATE_ADJUSTMENTS_COMPONENTS,
                                           i_year] = data.to_numpy()
    ...
    def _check_policy_parameter_ranges(self, policy_parameters, permitted_ranges):
        ...
        suffix_dict = {
            ...
            'c_corp_sec_179_expens_shares' : self.depreciation['c_corp_sec_179_expens_shares'],
            'pass_thru_sec_179_expens_shares' : self.depreciation['pass_thru_sec_179_expens_shares'],
            ...
            'c_corp_industry_adjustments' : self.tax_rate_adjustments['c_corp']['industry'],
            'pass_thru_industry_adjustments' : self.tax_rate_adjustments['pass_thru']['industry'],
            'c_corp_asset_adjustments' : self.tax_rate_adjustments['c_corp']['asset_type'],
            'pass_thru_asset_adjustments' : self.tax_rate_adjustments['pass_thru']['asset_type'],
        }

        for key in permitted_ranges:
            permitted_range_str = f"[{permitted_ranges[key]['min_value']}, {permitted_ranges[key]['max_value']}]"

            # First check parameter values already stored in numpy arrays, which are populated
            # based on parameters containing strings that reference file name suffixes in
            # the policy_parameters_* files
            if key in suffix_dict.keys():
                if key == 'straight_line_flags':
                    if not (
                        ...
                        | (suffix_dict[key] == 0.0)).all()
                    ):
                        raise ValueError(f"One of the parameter values in {key} is not within the set of permitted "
                    ...
                    if not (
                        ...
                        & ((suffix_dict[key] <= permitted_ranges[key]['max_value']) |
                           (suffix_dict[key] == 100000.0)).all()
                    ):
                        raise ValueError(f'One of the parameter values in {key} is not within the permitted '
            ...
            # Otherwise, skip parameter values that are strings containing names of file suffixes
                ...
                for val in policy_parameters[key].values:
                    if not (
                        ...
                        raise ValueError(f'The value for {key} ({val}) is not within the permitted range '
