...
class Writer():
    ...
    output_path : str
        String describing the folder path where output files for a given policy and perspective are stored.
    metrics : dict
        Dictionary including output variables, variables' names, variables' labels and variables' units.
    """
    ...
    def __init__(self, env, pol, agg, output, include_land=False, disp=None):
        ...
        vars = [self.agg.req_before_tax_returns,
        ...
        scaled_vars = [self.agg.req_before_tax_returns * 100,
        ...
        var_names = ['req_before_tax_returns',
        ...
        var_labels = ['Required Before-tax Returns',
        ...
        for var, scaled_var, var_name, var_label, unit in zip(vars, scaled_vars, var_names, var_labels, units):
    ...
    def write_all(self):
        ...
        This method calls five other methods:
            * self._write_supplemental_table_EMTRs() : Writes out an Excel file containing CBO's baseline supplemental
                table for effective marginal tax rates (EMTRs) on capital income.
            * self._write_summary_metrics() : Writes out values for all asset types or asset aggregates in
                the first year, or for all asset types by year.
            * self._write_values_biz() : Writes out values for a given variable by either industry or asset type
                (but always also include by legal form, and financing source as well) for businesses.
            * self._write_values_by_industry_asset_agg() : Writes out values for a given variable by industry and
                asset aggregates.
            * self._write_supplemental_table_tax_wedges() : Writes out an Excel file containing CBO's baseline supplemental
                table for differences in total tax wedges.
        ...
        if self.pol.policy_name == 'Current-Law' and self.pol.perspective == 'comprehensive':
            self._write_supplemental_table_EMTRs(self.metrics[id(self.output.total_EMTRs)])

        self._write_summary_metrics(self.metrics)

        if self.pol.perspective == 'comprehensive':
            self._write_values_biz(self.metrics[id(self.agg.weights)], by_var='asset_type')

        self._write_values_biz(self.metrics[id(self.output.total_EMTRs)], by_var='asset_type')
        self._write_values_biz(self.metrics[id(self.output.total_EMTRs)], by_var='industry')

        self._write_values_by_industry_asset_agg(self.metrics[id(self.output.total_EMTRs)],
                                                 asset_aggs = ['Nonresidential equipment',
        ...
        # Only write out tax wedge supplemental table for Current-Law under the uniformity perspective
        if self.pol.policy_name == 'Current-Law' and self.pol.perspective == 'uniformity':
            disp_metrics = {}
            disp_metrics['wgtd_avg_abs_diffs'] = self.disp.total_tax_wedge['wgtd_avg_abs_diffs']
            disp_metrics['wgtd_avg_abs_diffs']['wgtd_avg_abs_diff'] = (
                (disp_metrics['wgtd_avg_abs_diffs']['wgtd_avg_abs_diff'] * 100).round(decimals=2)
            )
            self._write_supplemental_table_tax_wedges(self.metrics[id(self.output.total_tax_wedges)],
                                                      disp_metrics['wgtd_avg_abs_diffs'])

        return None


    def _write_out_csv(self, df=None, filename=None, header_str=None, years=None, write_index=False):
        ...
        full_filename = f'{filename}_{years_str}_{self.pol.policy_name}_{self.pol.perspective}.csv'
        output_file = f'{self.output_path}{full_filename}'

        with open(output_file, 'w') as out_file:
            out_file.write(header_str + '\n')

        df.to_csv(output_file, mode='a', index=write_index, na_rep='NA', line_terminator='\n')
    ...
    def _write_summary_metrics(self, metrics=None):
        ...
        The first method makes a DataFrame containg summary output for key model metrics.
        The second method writes out the csv output file.
        ...
        header_str = (f"\"Key Model Metrics, by Asset Aggregate, Legal Form, and Financing Source, {START_YEAR} - {END_YEAR} "
        f"(returns and EMTRs are in percent; tax wedges are in percentage points; weights are in millions of dollars)\"")
        filename = 'summary'
        years = [START_YEAR, END_YEAR]

        # Write out file
        self._write_out_csv(df = summary_df,
                            filename = filename,
    ...
    def _make_summary_df(self, metrics):
        ...
        category_labels = [asset_aggs_labels,
        ...
        category_column_labels = ['asset_aggs',
                                  'legal_forms',
                                  'financing',
                                  'year']

        # Create a list that will contain a DataFrame with the method used and all the category labels
        dfs = []
        ...
        for key in metrics.keys():
            df[metrics[key]['var_name']] = metrics[key]['var'][slice].flatten().round(decimals=4)
    ...
    def _write_values_biz(self, var_dict, by_var, year=START_YEAR):
        ...
        for legal_form_abbreviation, legal_form_label in legal_forms_zip:
            ...
            if by_var == 'industry':
                var_slice = var_dict['var'][ALL_INDS_PLUS_AGG,
                                            INDEX[asset_agg_name],
                                            LEGAL_FORMS[legal_form_abbreviation],
                                            FINANCING_SOURCES['typical_equity'] : FINANCING_SOURCES['typical (biz)'] + 1,
                                            i_year]
                df_index = INDUSTRY_LABELS

            elif by_var == 'asset_type':
                var_slice = var_dict['var'][NUM_INDS,
    ...
    def _write_values_by_industry_asset_agg(self,
                                            var_dict,
                                            asset_aggs,
                                            legal_form_key,
                                            financing_key,
        ...
        for asset_agg in asset_aggs:
            ...
            df = pd.DataFrame(var_dict['var'][ALL_INDS_PLUS_AGG,
                                              INDEX[asset_agg],
    ...
    def _write_supplemental_table_EMTRs(self, EMTRs_dict):
        """Write out an Excel file containing CBO's baseline supplemental table for effective
        marginal tax rates (EMTRs) on capital income.
        ...
        supplemental_df, small_positives_exist, small_negatives_exist = (
            self._create_supplemental_EMTRs_df(EMTRs_dict)
        )

        supplemental_df.to_excel(writer,
        ...
        if small_positives_exist:
            worksheet.conditional_format('B12:L38', {'type' : 'cell',
                                                     'criteria' : 'between',
                                                     'minimum' : 0.001,
                                                     'maximum' : 0.049,
        ...
        worksheet.set_column(1, 12, 9, formats['data'])  # Set columns B:L to a width of 9

        worksheet.write(4, 0, 'Effective Marginal Tax Rates on Capital Income, by Tax Year', formats['bold'])
        ...
        worksheet.write(10, 0, 'All Assets Arising From New Investment', formats['bold'])
        ...
        worksheet.write(19, 0, 'Other intellectual property products', formats['indent2'])
        ...
        worksheet.write(37, 0, 'Owner-Occupied Housing, Including Land', formats['normal'])

        worksheet.write(40, 0, 'Data source: Congressional Budget Office.', formats['normal'])

        if small_positives_exist and small_negatives_exist:
            worksheet.write(42, 0, '* = between zero and 0.05 percentage points; ** = between -0.05 percentage points and zero.', formats['normal'])
        elif small_positives_exist:
            worksheet.write(42, 0, '* = between zero and 0.05 percentage points.', formats['normal'])
        elif small_negatives_exist:
            worksheet.write(42, 0, '** = between -0.05 percentage points and zero.', formats['normal'])
    ...
    def _create_supplemental_EMTRs_df(self, EMTRs_dict):
        """Create a DataFrame with the effective marginal tax rates (EMTRs) to be written to
        the supplemental table file.

        This method calls self._get_values() method.
        ...
        """
        all_assets_agg_with_land = 'All equipment, structures, IPP, inventories, and land'
        ...
        # Overall
        dfs_out.append(self._get_values(EMTRs_dict, NUM_INDS, all_assets_agg_without_land, 'biz+ooh', 'typical (biz+ooh)'))

        # Business assets
        dfs_out.append(self._get_values(EMTRs_dict, NUM_INDS, all_assets_agg_without_land, 'biz', 'typical (biz)'))
        ...
        for asset_agg in asset_aggs:
            dfs_out.append(self._get_values(EMTRs_dict, NUM_INDS, asset_agg, 'biz', 'typical (biz)'))

        # Business assets, by financing source
        dfs_out.append(self._get_values(EMTRs_dict, NUM_INDS, all_assets_agg_without_land, 'biz', 'typical_equity'))
        dfs_out.append(self._get_values(EMTRs_dict, NUM_INDS, all_assets_agg_without_land, 'biz', 'debt'))

        # Business assets, by legal form of organization
        dfs_out.append(self._get_values(EMTRs_dict, NUM_INDS, all_assets_agg_without_land, 'c_corp', 'typical (biz)'))
        dfs_out.append(self._get_values(EMTRs_dict, NUM_INDS, all_assets_agg_without_land, 'pass_thru', 'typical (biz)'))

        # Owner-occupied housing, by source of financing
        dfs_out.append(self._get_values(EMTRs_dict, OOH_IND, all_assets_agg_without_land, 'ooh', 'typical (biz+ooh)'))
        dfs_out.append(self._get_values(EMTRs_dict, OOH_IND, all_assets_agg_without_land, 'ooh', 'typical_equity'))
        dfs_out.append(self._get_values(EMTRs_dict, OOH_IND, all_assets_agg_without_land, 'ooh', 'debt'))

        ## Memorandum

        # Overall, including land
        dfs_out.append(self._get_values(EMTRs_dict, NUM_INDS, all_assets_agg_with_land, 'biz+ooh', 'typical (biz+ooh)'))

        # Business assets, including land
        dfs_out.append(self._get_values(EMTRs_dict, NUM_INDS, all_assets_agg_with_land, 'biz', 'typical (biz)'))

        # Business land
        dfs_out.append(self._get_values(EMTRs_dict, NUM_INDS, 'Land', 'biz', 'typical (biz)'))

        # Owner-occupied housing, including land
        dfs_out.append(self._get_values(EMTRs_dict, OOH_IND, all_assets_agg_with_land, 'ooh', 'typical (biz+ooh)'))

        supplemental_df = pd.concat(dfs_out, axis=1)

        small_positives_exist = ((supplemental_df > 0.001) & (supplemental_df < 0.049)).any(axis=None)
        small_negatives_exist = ((supplemental_df < -0.001) & (supplemental_df > -0.049)).any(axis=None)
    ...
    def _write_supplemental_table_tax_wedges(self, total_tax_wedges_dict, wgtd_avg_abs_diffs_dict):
        ...
        wgtd_avg_abs_diffs_dict : dict
            Dictionary containing information on weighted average of absolute differences
            in tax wedges to be written out.
        ...
        supplemental_df, small_positives_exist, small_negatives_exist = (
            self._create_supplemental_tax_wedges_df(total_tax_wedges_dict, wgtd_avg_abs_diffs_dict)
        )
        ...
        if small_positives_exist:
            worksheet.conditional_format('B11:L38', {'type' : 'cell',
                                                     'criteria' : 'between',
                                                     'minimum' : 0.001,
                                                     'maximum' : 0.049,
                                                     'format' : formats['asterisk']})
        ...
        worksheet.write(4, 0, 'Differences in Tax Wedges Among Types of New Investment, by Tax Year', formats['bold'])
        ...
        worksheet.write(16, 0, 'Difference between sources of financing', formats['italic_indent'])
        ...
        worksheet.write(22, 0, 'Difference between legal forms of organization', formats['italic_indent'])

        worksheet.write(24, 0, 'Weighted mean absolute difference between all asset pairs', formats['italic_indent'])
        worksheet.write(25, 0, 'Weighted mean absolute difference between all industry pairs', formats['italic_indent'])

        worksheet.write(27, 0, 'Tax Wedge for Owner-Occupied Housing', formats['bold'])
        worksheet.write(28, 0, 'Difference between owner-occupied housing and business assets', formats['italic_indent'])
        ...
        worksheet.write(34, 0, 'Difference between sources of financing', formats['italic_indent'])

        worksheet.write(36, 0, 'Memorandum', formats['bold'])
        worksheet.write(37, 0, 'Tax Wedge for Owner-Occupied Housing Structures', formats['normal'])
        worksheet.write(38, 0, 'Tax Wedge for Renter-Occupied Housing Structures', formats['normal'])
        worksheet.write(40, 0, 'Difference between owner- and renter-occupied housing structures', formats['italic_indent'])

        worksheet.write(43, 0, 'Data source: Congressional Budget Office.', formats['normal'])

        if small_positives_exist and small_negatives_exist:
            worksheet.write(45, 0, '* = between zero and 0.05 percentage points; ** = between -0.05 percentage points and zero.', formats['normal'])
        elif small_positives_exist:
            worksheet.write(45, 0, '* = between zero and 0.05 percentage points.', formats['normal'])
        elif small_negatives_exist:
            worksheet.write(45, 0, '** = between -0.05 percentage points and zero.', formats['normal'])
    ...
    def _create_supplemental_tax_wedges_df(self, total_tax_wedges_dict, wgtd_avg_abs_diffs_dict):
        """Create a DataFrame with the variables to be written to the supplemental table file with
        differences in tax wedges as measures of tax uniformity.
        ...
        dfs_out.append(self._get_values(total_tax_wedges_dict, NUM_INDS, all_assets_agg, 'biz', 'typical (biz)'))

        # Tax wedge for business assets, by source of financing
        dfs_out.append(self._get_values(total_tax_wedges_dict, NUM_INDS, all_assets_agg, 'biz', 'typical_equity'))
        dfs_out.append(self._get_values(total_tax_wedges_dict, NUM_INDS, all_assets_agg, 'biz', 'debt'))
        diff_biz_equity_debt = (
            pd.DataFrame((self._get_values(total_tax_wedges_dict, NUM_INDS, all_assets_agg, 'biz', 'typical_equity').values
                          - self._get_values(total_tax_wedges_dict, NUM_INDS, all_assets_agg, 'biz', 'debt').values),
                         index = YEARS)
        )
        dfs_out.append(diff_biz_equity_debt)

        # Tax wedge for business assets, by legal form of organization
        dfs_out.append(self._get_values(total_tax_wedges_dict, NUM_INDS, all_assets_agg, 'c_corp', 'typical (biz)'))
        dfs_out.append(self._get_values(total_tax_wedges_dict, NUM_INDS, all_assets_agg, 'pass_thru', 'typical (biz)'))
        diff_c_corp_pass_thru = (
            pd.DataFrame((self._get_values(total_tax_wedges_dict, NUM_INDS, all_assets_agg, 'c_corp', 'typical (biz)').values
                          - self._get_values(total_tax_wedges_dict, NUM_INDS, all_assets_agg, 'pass_thru', 'typical (biz)').values),
                         index = YEARS)
        )
        dfs_out.append(diff_c_corp_pass_thru)

        # Tax wedge for business assets, weighted average of absolute differences
        dfs_out.append(self._get_values_wgtd_avg_abs_diffs(wgtd_avg_abs_diffs_dict, 'biz', 'assets', all_assets_agg))
        dfs_out.append(self._get_values_wgtd_avg_abs_diffs(wgtd_avg_abs_diffs_dict, 'biz', industry_dim_name))

        ## Tax wedge for owner-occupied housing
        dfs_out.append(self._get_values(total_tax_wedges_dict, OOH_IND, all_assets_agg, 'ooh', 'typical (biz+ooh)'))

        # Difference between owner-occupied housing and business assets
        diff_ooh_biz = (
            pd.DataFrame((self._get_values(total_tax_wedges_dict, OOH_IND, all_assets_agg, 'ooh', 'typical (biz+ooh)').values
                          - self._get_values(total_tax_wedges_dict, NUM_INDS, all_assets_agg, 'biz', 'typical (biz)').values),
                         index = YEARS)
        )
        dfs_out.append(diff_ooh_biz)

        # Tax wedge for owner-occupied housing, by source of financing
        dfs_out.append(self._get_values(total_tax_wedges_dict, OOH_IND, all_assets_agg, 'ooh', 'typical_equity'))
        dfs_out.append(self._get_values(total_tax_wedges_dict, OOH_IND, all_assets_agg, 'ooh', 'debt'))
        diff_ooh_equity_debt = (
            pd.DataFrame((self._get_values(total_tax_wedges_dict, OOH_IND, all_assets_agg, 'ooh', 'typical_equity').values
                          - self._get_values(total_tax_wedges_dict, OOH_IND, all_assets_agg, 'ooh', 'debt').values),
                         index = YEARS)
        )
        dfs_out.append(diff_ooh_equity_debt)

        ## Memorandum
        dfs_out.append(self._get_values(total_tax_wedges_dict, OOH_IND, 'Residential buildings', 'ooh', 'typical (biz+ooh)'))
        dfs_out.append(self._get_values(total_tax_wedges_dict, REAL_ESTATE_IND, 'Residential buildings', 'biz', 'typical (biz)'))
        diff_ooh_roh = (
            pd.DataFrame((self._get_values(total_tax_wedges_dict, OOH_IND, 'Residential buildings', 'ooh', 'typical (biz+ooh)').values
                          - self._get_values(total_tax_wedges_dict, REAL_ESTATE_IND, 'Residential buildings', 'biz', 'typical (biz)').values),
                         index = YEARS)
        )
    ...
    def _get_values(self, var_dict, industry_index, asset_agg, legal_form, financing_source):
        """Get values to write out by asset aggregation, legal form, and financing source.
        ...
        """
        col_names = pd.MultiIndex.from_product([[asset_agg],
                                                [legal_form],
                                                [financing_source]])

        col = pd.DataFrame(var_dict['var'][industry_index,
                                           INDEX[asset_agg],
    ...
    def _get_values_wgtd_avg_abs_diffs(self, var_dict, legal_form, dimension, asset_agg=None):
        """Get values to write out by legal form and dimension along which weighted average
        of absolute differences is calculated.
        ...
        -------
        col : pd.DataFrame
            A single column pandas DataFrame with a 3 row header indicating the dimension
            along which to get weighted average of absolute differences, the legal form,
            and a label for the weighted average considered.

        """
        if dimension == 'assets':
            if asset_agg is None:
                raise ValueError('Asset aggregate must be specified when dimension = "assets"')
            agg_name = asset_agg
            label = 'Difference Among Asset Types'
        elif dimension in ['industries (excluding land)', 'industries (including land)']:
            agg_name = 'All Industries'
            label = 'Difference Among Industries'
        else:
            raise ValueError('dimension must be equal to "assets", "industries (excluding land)" '
                             'or "industries (including land)"')

        var = var_dict[(var_dict['dimension'] == dimension)]

        col_names = pd.MultiIndex.from_product([[agg_name],
                                                [legal_form],
                                                [label]])

        col = pd.DataFrame(data=np.array([var.loc[(var['aggregation'] == agg_name)
                                          & (var['legal_form'] == legal_form),
                                          'wgtd_avg_abs_diff']]).transpose(),
                           index = YEARS,
                           columns = col_names)
    ...
    def _create_formats(self, workbook):
        ...
        formats = {
         ...
         'asterisk' : workbook.add_format({
            'num_format' : '"*";"*"'
         }),

         'double_asterisk' : workbook.add_format({
            'num_format' : '"**";"**"'
         }),
