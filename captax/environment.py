...
class Environment():
    ...
    environment_path : str
        String describing the folder path where environment parameters input files are stored.
    environment_parameters_permitted_ranges_file : str
        String describing the file path of file storing permitted ranges for environment parameters.
    env : pd.DataFrame
    ...
    debt_shares_df : pd.DataFrame
        DataFrame with unscaled debt-financing shares by legal form (C corps, pass-throughs and
        owner-occupied housing).
    shares : dict
        Dictionary including rescaled equity and debt-financing shares for businesses and owner-occupied
        housing, and shares of C corp equity investments financed by new equity and retained earnings.
    ...
    def __init__(self):
        ...
        self.environment_parameters_permitted_ranges_file = f'{self.environment_path}/environment_parameters_permitted_ranges.csv'
        ...
        # Aggregate debt shares
        self.agg_debt_share = {
            'financial_sector' : self.env.iloc[0]['financial_sector_debt_share'].round(decimals=4),
            'nonfin_c_corp' : self.env.iloc[0]['nonfinancial_c_corp_debt_share'].round(decimals=4),
            'nonfin_pass_thru' : self.env.iloc[0]['nonfinancial_pass_thru_debt_share'].round(decimals=4),
            'ooh' : self.env.iloc[0]['ooh_debt_share'].round(decimals=4)
        }

        ooh_shares = {
            'debt' : self.env.iloc[0]['ooh_debt_share'].round(decimals=4),
            'typical_equity' : 1.0 - self.env.iloc[0]['ooh_debt_share'].round(decimals=4)
        }

        # C corp equity financing shares
        c_corp_equity_shares = {
            'retained_earnings' : self.env.iloc[0]['c_corp_equity_retained_earnings_share'],
            'new_equity' : 1.0 - self.env.iloc[0]['c_corp_equity_retained_earnings_share']
        }

        # Rates of return
        nominal_rate_of_return_equity = self.env.iloc[0]['nominal_rate_of_return_equity']
        ...
        if not (self.cap_gains_holding_period['long_term'] > self.cap_gains_holding_period['short_term']):
            raise ValueError(f'Parameter value of cap_gains_long_term_holding_period has to be larger than '
                             f'value of cap_gains_short_term_holding_period')

        if not (self.cap_gains_holding_period['at_death'] > self.cap_gains_holding_period['long_term']):
            raise ValueError(f'Parameter value of cap_gains_at_death_term_holding_period has to be larger '
        ...
        #---------------------------------------------------------------------------------
        self.econ_depreciation_detailed_industry = self._read_econ_depreciation('economic_depreciation.csv')
        ...
        equity_shares[:NUM_BIZ_INDS, :NUM_ASSETS, LEGAL_FORMS['ooh']]= np.zeros((NUM_BIZ_INDS, NUM_ASSETS))

        financing_shares = {
            'debt' : debt_shares,
            'typical_equity' : equity_shares
        }
        ...
        #---------------------------------------------------------------------------------
        self.permitted_ranges = pd.read_csv(self.environment_parameters_permitted_ranges_file,
                                            index_col='environment_parameter').to_dict('index')
    ...
    def _read_economic_environment(self, filename):
        ...
        df = pd.read_csv(self.environment_path + filename, index_col='parameter').transpose()
    ...
    def _calc_debt_share_rescaling_factors(self, agg_debt_share):
        """Calculate rescaling factors applied to debt-financing shares by industry and legal form.

        Rescaling factors are calculated at an aggregate level for:
            1) financial sector,
            2) nonfinancial C corps, and
            3) nonfinancial pass-throughs
        by dividing the aggregate debt-financing shares specified in the environment parameters
        input file by the hard-coded values for aggregate debt shares specified at the bottom of
        `constants.py`.
        ...
        rescaling_factors : dict
            Aggregate rescaling factors applied to the financial sector, nonfinancial C corps, and
            nonfinancial pass-throughs.

        """
        rescaling_factors = {
            'financial_sector' : agg_debt_share['financial_sector'] / AGG_DEBT_SHARE['financial_sector'],
            'nonfin_c_corp' : agg_debt_share['nonfin_c_corp'] / AGG_DEBT_SHARE['nonfin_c_corp'],
            'nonfin_pass_thru' : agg_debt_share['nonfin_pass_thru'] / AGG_DEBT_SHARE['nonfin_pass_thru']
        }
    ...
    def _rescale_debt_shares(self, debt_shares, rescaling_factors):
        """Rescale debt-financing shares by industry and legal form using aggregate rescaling
        factors returned by self._calc_debt_share_rescaling_factors() method.
        ...
        rescaled_debt_shares : np.ndarray
            Array containing debt-financing shares by industry and legal form, after rescaling.

        Note
        ----
        Only rescales debt-financing shares if agg_debt_share parameters have changed relative to
        the hard-coded values at the bottom of `constants.py`.

        """
        # Only rescale debt-financing shares if self.agg_debt_share is different from AGG_DEBT_SHARE
        if self.agg_debt_share != AGG_DEBT_SHARE:

            # Rescale debt shares by industry based scaling factors.
            debt_shares.loc[ALL_FINANCIAL_INDS, 'c_corp'] *= rescaling_factors['financial_sector']
            debt_shares.loc[ALL_NONFINANCIAL_INDS, 'c_corp'] *= rescaling_factors['nonfin_c_corp']
            debt_shares.loc[ALL_FINANCIAL_INDS, 'pass_thru'] *= rescaling_factors['financial_sector']
            debt_shares.loc[ALL_NONFINANCIAL_INDS, 'pass_thru'] *= rescaling_factors['nonfin_pass_thru']

            # Make sure debt shares do not exceed 1.0 after applying the rescaling factors
        ...
        rescaled_debt_shares = np.zeros((NUM_INDS, NUM_ASSETS, NUM_FOR_PROFIT_LEGAL_FORMS))

        for legal_form in ['c_corp','pass_thru','ooh']:
            rescaled_debt_shares[:NUM_INDS, :NUM_ASSETS, LEGAL_FORMS[legal_form]] = (
    ...
    def _check_environment_parameter_ranges(self, environment_parameters, permitted_ranges):
        """Check that all environment parameters are within permitted ranges.

        Parameters
        ----------
        environment_parameters : pd.DataFrame
            DataFrame containing all the environment parameters that are dimensioned by year.
        permitted_ranges : dict
        ...
        for key in permitted_ranges:
            ...
            if np.isnan(permitted_ranges[key]['min_value']) and np.isnan(permitted_ranges[key]['max_value']):
                continue
            ...
            # and then process the remaining parameters
