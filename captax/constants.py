...
FINANCING_SOURCES['typical_equity'] = 2         # weighted average of new_equity and retained_earnings
...
ALL_NONRES_STRUCTURES = ALL_STRUCTURES = np.r_[START_STRUCTURES : END_STRUCTURES]
...
ALL_ENTERTAINMENT = np.r_[ALL_RESEARCH[-1] + 1 : ALL_RESEARCH[-1] + 1 + NUM_ENTERTAINMENT]
ALL_NON_BEA_INTANGIBLES = np.r_[ALL_ENTERTAINMENT[-1] + 1 : ALL_ENTERTAINMENT[-1] + 1 + NUM_NON_BEA_INTANGIBLES]

ALL_NON_RESEARCH_IPP = np.r_[ALL_MINERAL, ALL_SOFTWARE, ALL_ENTERTAINMENT, ALL_NON_BEA_INTANGIBLES]

# Define other asset aggregates to use
...
ALL_NON_RESEARCH_IPP_MINUS_MINERAL = [x for x in ALL_NON_RESEARCH_IPP if x not in ALL_MINERAL]

ALL_EQUIP_STRUCT = np.r_[ALL_EQUIPMENT, ALL_STRUCTURES_PLUS_MINERAL]
ALL_EQUIP_STRUCT_INVENT = np.r_[ALL_EQUIPMENT, ALL_STRUCTURES_PLUS_MINERAL, ALL_INVENTORIES]
ALL_EQUIP_STRUCT_INVENT_LAND = np.r_[ALL_EQUIPMENT, ALL_STRUCTURES_PLUS_MINERAL, ALL_INVENTORIES, ALL_LAND]
ALL_EQUIP_STRUCT_IPP = np.r_[ALL_EQUIPMENT, ALL_STRUCTURES, ALL_IPP]
ALL_EQUIP_STRUCT_IPP_INVENT = np.r_[ALL_EQUIPMENT, ALL_STRUCTURES, ALL_IPP, ALL_INVENTORIES]

# Main asset aggregates
ASSET_AGGS = (ALL_NONRES_EQUIPMENT,
...
# Index positions for specific asset types and asset aggregates
#-----------------------------------------------------------------------------------------
INDEX = {}

INDEX['Residential buildings'] = NUM_EQUIPMENT + NUM_STRUCTURES + NUM_IPP + NUM_NON_BEA_INTANGIBLES + 1
INDEX['Inventories'] = NUM_EQUIPMENT + NUM_STRUCTURES + NUM_IPP + NUM_NON_BEA_INTANGIBLES + NUM_RESIDENTIAL
INDEX['Land'] = NUM_EQUIPMENT + NUM_STRUCTURES + NUM_IPP + NUM_NON_BEA_INTANGIBLES + NUM_RESIDENTIAL + NUM_INVENTORIES

# Fill in index values for asset aggregates
for i in ALL_ASSET_AGGS:
    INDEX[ASSET_TYPE_LABELS[i]] = i


ALL_OOH_ASSETS = np.r_[INDEX['Residential buildings'], INDEX['Land']]

# Components of asset type/industry adjustments applied to tax rates
#-----------------------------------------------------------------------------------------
TAX_RATE_ADJUSTMENTS_COMPONENTS = {'eligibility' : 0,
                                   'rate' : 1}
