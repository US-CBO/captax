...
class Aggregator():
    ...
    def aggregate_all(self):
        """Aggregate weights, before-tax rates of return, after-tax rates of return required by savers
        and after-tax rates of return required by investors.

        This method calls self._aggregate_variable(), which is used to aggregate weights, before-tax rates of
        return and after-tax rates of return required by savers and by investors.
        ...
        self.req_before_tax_returns = self._aggregate_variable(self.calc.req_before_tax_returns,
                                                               ASSET_AGGS,
                                                               self.weights)

        self.req_after_tax_returns_savers = (
            self._aggregate_variable(self.calc.req_after_tax_returns_savers[:NUM_INDS,
                                                                            :NUM_ASSETS,
        ...
        self.req_after_tax_returns_investors = (
            self._aggregate_variable(self.calc.req_after_tax_returns_investors,
                                     ASSET_AGGS,
                                     self.weights)
        )
    ...
    def _aggregate_variable(self, in_var, asset_aggs, weights=None):
        ...
        if weights is None:
            ...
            # Aggregate values
            values_by_asset_type = self._calc_values_by_asset_type_comprehensive(wgts, ones)
            values_by_asset_agg = self._calc_values_by_asset_agg_comprehensive(wgts, ones, asset_aggs)
            ...
            if self.pol.perspective == 'comprehensive':
                values_by_asset_type = self._calc_values_by_asset_type_comprehensive(var, weights)
                values_by_asset_agg = self._calc_values_by_asset_agg_comprehensive(var, weights, asset_aggs)

            elif self.pol.perspective == 'uniformity':
                values_by_asset_type = self._calc_values_by_asset_type_uniformity(var, weights)
                values_by_asset_agg = self._calc_values_by_asset_agg_uniformity(var, weights, asset_aggs)
        ...
        for assets, values in assets_values:
            slice = np.ix_(ALL_INDS_PLUS_AGG,
                           assets,
                           ALL_LEGAL_FORMS,
    ...
    def _calc_values_by_asset_type_comprehensive(self, in_var, weights):
        ...
        out_array[:NUM_INDS,
                  ...
                  :NUM_FOR_PROFIT_LEGAL_FORMS,
                  :NUM_FINANCING_SOURCES,
                  :NUM_YEARS] = in_var[:NUM_INDS,
                                       :NUM_ASSETS,
                                       :NUM_FOR_PROFIT_LEGAL_FORMS,
                                       :NUM_FINANCING_SOURCES,
                                       :NUM_YEARS]

        equity_and_debt = slice(FINANCING_SOURCES['typical_equity'], FINANCING_SOURCES['debt'] + 1)
        ...
        financing_aggregates = [FINANCING_SOURCES['typical (biz)'], FINANCING_SOURCES['typical (biz+ooh)']]
        aggregates_components_financing = zip(form_aggregates, form_components, financing_aggregates)
    ...
    def _calc_values_by_asset_type_uniformity(self, in_var, weights):
        ...
        equity_and_debt = slice(FINANCING_SOURCES['typical_equity'], FINANCING_SOURCES['debt'] + 1)
        len_equity_and_debt = (equity_and_debt.indices(LEN_FINANCING_SOURCES)[1]
                              - equity_and_debt.indices(LEN_FINANCING_SOURCES)[0])

        # Industry aggregates, by asset type, legal form, financing source and year
        # --------------------------------------------------------------------------------
        # Businesses
        out_array[NUM_INDS, :NUM_ASSETS, :NUM_BIZ, :NUM_FINANCING_SOURCES, :NUM_YEARS] = (
            (in_var[:NUM_BIZ_INDS, :NUM_ASSETS, :NUM_BIZ, :NUM_FINANCING_SOURCES, :NUM_YEARS]
             * (np.tile(weights[:NUM_BIZ_INDS,
            ...
            / np.tile(weights[NUM_INDS,
                              INDEX[asset_agg_name],
                              LEGAL_FORMS['biz'],
                              FINANCING_SOURCES['typical (biz)'], :NUM_YEARS],
        ...
        form_components = [slice(0, NUM_BIZ), slice(0, NUM_FOR_PROFIT_LEGAL_FORMS)]
        financing_aggregates = [FINANCING_SOURCES['typical (biz)'], FINANCING_SOURCES['typical (biz+ooh)']]
        ind_form_financing = zip(ind_components, form_aggregates, form_components, financing_aggregates)

        for ind_comps, form_agg, form_comps, financing_agg in ind_form_financing:

            len_ind_comps = (ind_comps.indices(NUM_INDS)[1]
                             - ind_comps.indices(NUM_INDS)[0])

            len_form_comps = (form_comps.indices(LEN_LEGAL_FORMS)[1]
                             - form_comps.indices(LEN_LEGAL_FORMS)[0])

            # Legal form aggregates, by industry, asset type, financing source and year
            # --------------------------------------------------------------------------------
            out_array[ind_comps, :NUM_ASSETS, form_agg, :NUM_FINANCING_SOURCES, :NUM_YEARS] = (
                (in_var[ind_comps, :NUM_ASSETS, form_comps, :NUM_FINANCING_SOURCES, :NUM_YEARS]
                 * (np.tile(weights[NUM_INDS,
                                    INDEX[asset_agg_name],
                                    form_comps,
                                    financing_agg,
            ...
            out_array[ind_comps, :NUM_ASSETS, form_agg, financing_agg, :NUM_YEARS] = (
                ...
                / (np.tile(np.square(weights[NUM_INDS,
                                             INDEX[asset_agg_name],
                                             form_agg,
                                             financing_agg,
                                             :NUM_YEARS]),
                           (len_ind_comps, NUM_ASSETS, 1))).transpose((0,1,2))
            )
            ...
            out_array[NUM_INDS, :NUM_ASSETS, form_comps, financing_agg, :NUM_YEARS] = (
                ...
                / (np.tile(np.square(weights[NUM_INDS,
                                             ...
                                             financing_agg,
                                             :NUM_YEARS]),
                           (len_form_comps, NUM_ASSETS, 1))).transpose((1,0,2))
            )

            # Industry and legal form aggregates, by asset type, financing source and year
            # --------------------------------------------------------------------------------
            out_array[NUM_INDS, :NUM_ASSETS, form_agg, :NUM_FINANCING_SOURCES, :NUM_YEARS] = (
                (in_var[ind_comps, :NUM_ASSETS, form_comps, :NUM_FINANCING_SOURCES, :NUM_YEARS]
                 * (np.tile(weights[ind_comps,
                                    INDEX[asset_agg_name],
                                    form_agg,
                ...
                / (np.tile(np.square(weights[NUM_INDS,
                                             ...
                                             financing_agg,
                                             :NUM_YEARS]),
                           (NUM_FINANCING_SOURCES, NUM_ASSETS, 1))).transpose((1,0,2))
            )
            ...
            out_array[NUM_INDS, :NUM_ASSETS, form_agg, financing_agg, :NUM_YEARS] = (
                ...
                / (np.tile(np.power(weights[NUM_INDS,
                                            INDEX[asset_agg_name],
                                            form_agg,
                                            financing_agg,
                                            :NUM_YEARS], 3),
                           (NUM_ASSETS, 1))).transpose((0,1))
            )
    ...
    def _calc_values_by_asset_agg_uniformity(self, in_var, weights, asset_aggs):
        ...
        and applying weights that vary by industry, asset, legal form, and financing source.
        ...
        for asset_agg_range in asset_aggs:
            ...
            # Owner-occupied housing
            if INDEX['Land'] in asset_agg_range and INDEX['Residential buildings'] in asset_agg_range:
                out_array[OOH_IND,
                          NUM_ASSETS + output_position,
                          LEGAL_FORMS['ooh'],
                          :NUM_FINANCING_SOURCES,
                          :NUM_YEARS] = (
                               ...
                               * (np.tile(weights[NUM_INDS,
                               ...
                               / np.tile((weights[NUM_INDS,
                                                  ...
                                                  LEGAL_FORMS['biz+ooh'],
                                                  FINANCING_SOURCES['typical (biz+ooh)'],
                                                  :NUM_YEARS]).sum(axis=(0)),
                                         (NUM_FINANCING_SOURCES, 1))
                )

            elif INDEX['Residential buildings'] in asset_agg_range:
                out_array[OOH_IND,
                          NUM_ASSETS + output_position,
                          LEGAL_FORMS['ooh'],
                          :NUM_FINANCING_SOURCES,
            ...
            # ----------------------------------------------------------------------------
            # Businesses
            for legal_form, financing_source in [('biz','typical (biz)'), ('biz+ooh','typical (biz+ooh)')]:
                out_array[:NUM_BIZ_INDS,
                          NUM_ASSETS + output_position,
                          :NUM_BIZ, FINANCING_SOURCES[financing_source],
                          :NUM_YEARS] = (
                              (in_var[:NUM_BIZ_INDS,
                                      asset_agg_range,
                                      :NUM_BIZ,
            ...
            for legal_form in ['biz', 'biz+ooh']:
                out_array[:NUM_BIZ_INDS,
                          ...
                          :NUM_YEARS] = (
                               ...
                               * (np.tile(weights[NUM_INDS,
                                                  NUM_ASSETS + output_position,
                                                  :NUM_BIZ, FINANCING_SOURCES['typical (biz)'],
                                                  :NUM_YEARS],
                                          (NUM_BIZ_INDS, len(asset_agg_range), NUM_FINANCING_SOURCES, 1, 1))
                                  ).transpose((0,1,3,2,4))
                               ).sum(axis=(1, 2))
                               / np.tile(np.square(weights[NUM_INDS,
                                                           NUM_ASSETS + output_position,
            ...
            if INDEX['Land'] in asset_agg_range or INDEX['Residential buildings'] in asset_agg_range:
                out_array[NUM_INDS,
                          NUM_ASSETS + output_position,
                          LEGAL_FORMS['biz+ooh'],
                          :NUM_FINANCING_SOURCES,
            ...
            # All businesses + owner-occupied housing, when accounting for weights across all businesses
            # + owner-occupied housing
            if INDEX['Land'] in asset_agg_range or INDEX['Residential buildings'] in asset_agg_range:
                out_array[NUM_INDS,
                          NUM_ASSETS + output_position,
