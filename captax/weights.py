...
class Weights:
    ...
    detailed_industry_weights : np.ndarray
        Detailed industry weights for aggregating detailed industries into broader
        industries.
    ...
    def __init__(self, shares_data):
        ...
        This method calls four other methods:
            * self._read_assets_data()
              Reads assets data used as the basis for calculating model weights.

            * self._read_asset_shares()
              Reads in shares of total assets by industry and asset type for each legal
              form.

            * self._calc_weights()
              Calculates weights by industry, asset type, legal form and financing
              source.

            * self._read_detailed_industry_weights()
              Reads detailed industry weights to apportion industry data to detailed
              industry level.

        Parameters
        ----------
        shares_data : dict
            Dictionary including equity and debt-financing shares for businesses and
            owner-occupied housing, and shares of C corp equity investments financed by
        ...
        if not ((_c_corp_shares[OOH_IND, :NUM_ASSETS] == 0).all()):
            raise ValueError(
                f"C corp shares of total assets must be equal to 0 for the OOH industry"
            )

        _pass_thru_shares = self._read_asset_shares("asset_shares_pass_thru.csv")
        if not ((_pass_thru_shares[OOH_IND, :NUM_ASSETS] == 0).all()):
            raise ValueError(
                f"Pass-through shares of total assets must be equal to 0 for the OOH industry"
            )

        if not ((_c_corp_shares + _pass_thru_shares <= 1).all()):
            raise ValueError(
                f"The sum of C corp and pass-through shares of total assets must be lower or equal to 1"
            )
        _nonprofit_shares = 1.0 - _c_corp_shares - _pass_thru_shares

        _ooh_shares = self._read_asset_shares("asset_shares_ooh.csv")
        if not (
            (_ooh_shares[ALL_BIZ_INDS, :NUM_ASSETS] == 0).all()
            & (_ooh_shares[OOH_IND, :NUM_ASSETS] == 1).all()
        ):
            raise ValueError(
                f"OOH shares of total assets must be equal to 0 for all industries other than the "
        ...
        self.detailed_industry_weights = self._read_detailed_industry_weights(
    ...
    def _read_asset_shares(self, filename):
        """Read in shares of total assets by industry and asset type for each legal
        form.
        ...
        The owner-occupied housing matrix is all zeros, except for the last row of the
        matrix, which includes information on assets by asset type for owner-occupied
        housing.
        ...
        Note
        ----
        There are three asset shares files: C corporations, pass-throughs, and
        owner-occupied housing. Each file contains an asset type by industry matrix,
        which represents the share of total assets within that legal form.
    ...
    def _calc_weights(self, assets, asset_shares, shares):
        ...
        share parameters by financing sources (debt/equity shares and the share of C
        corps equity that is financed through new equity and retained earnings) and by
        legal form.
        ...
        # ------------------------------------------------------------------------------
        weights = np.zeros(
            (NUM_INDS, NUM_ASSETS, LEN_LEGAL_FORMS, LEN_FINANCING_SOURCES)
        )
            ...
            for financing_source in ["typical_equity", "debt", "typical (biz)"]:
                ...
                if legal_form == "c_corp" and financing_source == "typical_equity":
                    for equity_financing_source in ["new_equity", "retained_earnings"]:
                        weights[
                            ...
                            * shares["financing"][financing_source][
                ...
                if financing_source == "typical (biz)":
                    weights[
                        ...
                        LEGAL_FORMS[legal_form],
                        FINANCING_SOURCES[financing_source],
                    ] = (assets * asset_shares[legal_form])

        # Weights for typically financed investments (businesses + owner-occupied
        # housing)
        weights[
            :NUM_INDS,
            :NUM_ASSETS,
            ...
            :NUM_INDS, :NUM_ASSETS, :LEN_LEGAL_FORMS, FINANCING_SOURCES["typical (biz)"]
        ]

        return weights

    def _read_detailed_industry_weights(self, filename):
        """Read detailed industry weights to apportion industry data to detailed
        industry level.
        ...
        file = self.weights_path + filename
        df = pd.read_csv(file, skiprows=1, index_col="detailed_industry").round(
            decimals=3
        )
        ...
        if not (np.all((sum_detailed_industry_weights == 1.0))):
            raise ValueError(
                f"Detailed industry weights do not add up to 1 for at least one industry"
            )
