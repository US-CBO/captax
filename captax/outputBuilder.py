...
class OutputBuilder():
    ...
    def build_all(self):
        ...
        self.c_corp_tax_wedges = self._calc_c_corp_tax_wedges(self.agg.req_before_tax_returns,
                                                              self.agg.req_after_tax_returns_investors)
        self.total_tax_wedges = self._calc_total_tax_wedges(self.agg.req_before_tax_returns,
                                                            self.agg.req_after_tax_returns_savers)

        print('* Tax wedges calculated')

        self.c_corp_EMTRs = self._calc_effective_marginal_tax_rates(self.c_corp_tax_wedges,
                                                                    self.agg.req_before_tax_returns)
        self.total_EMTRs = self._calc_effective_marginal_tax_rates(self.total_tax_wedges,
                                                                   self.agg.req_before_tax_returns)
    ...
    def _calc_c_corp_tax_wedges(self, req_before_tax_returns, req_after_tax_returns_investors):
        """Calculate tax wedge for C corporations by industry, asset type, financing source,
        and year. The C corporation tax wedge is equal to the required before-tax rate of return
        minus the required after-tax rate of return to investors.
        ...
        """
        # Initialize array
        c_corp_tax_wedges = np.zeros((LEN_INDS,
                                      LEN_ASSETS,
                                      LEN_LEGAL_FORMS,
                                      LEN_FINANCING_SOURCES,
                                      NUM_YEARS))
        ...
        c_corp_tax_wedges[:LEN_INDS, :LEN_ASSETS, LEGAL_FORMS['c_corp'], :LEN_FINANCING_SOURCES, :NUM_YEARS] = (
            req_before_tax_returns[:LEN_INDS,
                                   :LEN_ASSETS,
                                   LEGAL_FORMS['c_corp'],
                                   :LEN_FINANCING_SOURCES,
                                   :NUM_YEARS]
            - req_after_tax_returns_investors[:LEN_INDS,
                                              :LEN_ASSETS,
                                              LEGAL_FORMS['c_corp'],
                                              :LEN_FINANCING_SOURCES,
                                              :NUM_YEARS]
    ...
    def _calc_total_tax_wedges(self, req_before_tax_returns, req_after_tax_returns_savers):
        """Calculate total tax wedge by industry, asset type, legal form, financing source
        and year.

        The total tax wedge is equal to the required before-tax rates of return minus the
    ...
    def _calc_effective_marginal_tax_rates(self, tax_wedges, req_before_tax_returns):
        ...
        effective_marginal_tax_rates = np.zeros((LEN_INDS,
                                                 LEN_ASSETS,
                                                 LEN_LEGAL_FORMS,
                                                 LEN_FINANCING_SOURCES,
                                                 NUM_YEARS))
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            effective_marginal_tax_rates = (
                np.where(req_before_tax_returns != 0.0,
